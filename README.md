...
`python3 tests/test_physical_media.py` — 69 node-driver assertions plus
...
(label/disc/case faces), label/disc render-kind classification,
generated-face helpers (spine text, abbr), and template geometry sanity.

Guards: authored SVG templates present, parseable, and QtSvg-safe;
renderers compose the templates (no primitive-drawn objects); GameTile
is pure box-art; no `gba`/`ps2` literals outside `MediaTemplates.js`;
new QML uses only existing `CrystalTheme.js` tokens; no 3D imports; no
Socket references; no new audio subsystem; launch still flows through
`GameLibrary.launchCurrent`; brace balance on new/modified QML; JS
import-alias consistency; Inspect input safety net (watchdog + B
escape + `open()` success flag).
